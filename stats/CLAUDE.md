...
⚠️ `perf-rankings.json` (outil `ranking/`) suit la même chaîne : `build-perf-rankings.cjs` est exécuté dans le cron `build-index.yml` (après les index), commité dans `bmx-race-stats`, puis copié vers `ranking/perf-rankings.json` par `sync-index-metas.yml`. Régénération manuelle possible : `node tools/build-perf-rankings.cjs` (dans `../../sqorz_stats`), puis recopier vers `ranking/perf-rankings.json` via `club/tools/sync-data.sh` ou un commit direct.
...
1. **Rangée 1 — parties** : `📊 Global | 📍 Régional | 🇫🇷 National | 🇪🇺 UEC | 🌍 UCI` (constante `LEVELS` dans `index.html`, clés `global|regional|national|uec|uci`).
   ...
   - Une partie n'est affichée que si le pilote actif a des données à ce niveau (badge = nombre d'engagements). **On démarre toujours sur Global** à l'ouverture/à chaque nouvelle recherche (`currentPart = null` → première partie disponible) ; le choix de partie reste en mémoire pour la session courante (`currentPart`, survit aux re-rendus tri/refresh) mais n'est **plus** persisté entre sessions (l'ancienne clé `partPref` n'est plus ni lue ni écrite). Le mini-sélecteur de la comparaison 2 pilotes propose les mêmes parties (Global compris).
...
| `build-uciworldcup.js` | Script de génération de l'index Coupe du monde UCI (`uci-worldcup-index.json`) depuis l'API uci.org — crawl Node, cache `.cache/uciworldcup/`, spec `docs/superpowers/specs/2026-09-14-uci-worldcup-design.md` |
