...
const BF = require(path.join(root, 'stats', 'build-field.js'));
const BF_SRC = fs.readFileSync(path.join(root, 'stats', 'build-field.js'), 'utf8');
const SC = new Function('window', fs.readFileSync(path.join(root, 'common.js'), 'utf8') + '\nreturn window.BmxCommon;')({});
const html = fs.readFileSync(path.join(root, 'stats', 'index.html'), 'utf8');
...
test('build sans coefs : K partagé, pas de NATIONAL/COEFS', () => {
  ...
  for (const f of ['build-index.js', 'build-uec.js']) {
    const src = fs.readFileSync(path.join(root, 'stats', f), 'utf8');
